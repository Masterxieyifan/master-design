...
import { cloneDeep, isString, isObject, isEmpty, isArray, filterEmptyAttrs } from '@master_vantop/vantop-util';
...
export const operateStorage = {
    ...
    methods: {
        ...
        // 预处理查询参数
        preHandleParams(params, setting = {}) {
            const {
                filterEmpty = false,
                trimString = true
            } = setting;
            ...
            let queryParams = Object.assign(
                {},
                this.$refs?.['advancedSearch']?.queryParams || this.query,
                params,
            );

            if (selectInputKey) {
                // 拆解快捷搜索参数值
                const [typeKey, valKey] = selectInputKey.split('-');
                let [type, val] = queryParams[selectInputKey];
                delete queryParams[selectInputKey];
                queryParams[typeKey] = type;
                queryParams[valKey] = val;
            }

            // 是否过滤掉空值属性
            if (filterEmpty) {
                queryParams = filterEmptyAttrs(queryParams);
            }
